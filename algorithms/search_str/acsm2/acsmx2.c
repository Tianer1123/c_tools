		...
		)
{
	...
	{
		if (acsm->acsmNumStates < 0xff)
		{
			acsm->sizeofstate = 1;
			summary.num_1byte_instances++;
		}
		else if (acsm->acsmNumStates < 0xffff)
		{
			acsm->sizeofstate = 2;
...
void acsmx2_print_qinfo(void)
{
	...
	{
		printf("mpse: queue size     = %d, max possible = %d\n", snort_conf->max_inq, AC_MAX_INQ);
		printf("mpse: queue flushes  = " STDu64 "\n", snort_conf->tot_inq_flush );
		printf("mpse: queue inserts  = " STDu64 "\n", snort_conf->tot_inq_inserts );
		printf("mpse: queue uinserts = " STDu64 "\n", snort_conf->tot_inq_uinserts );
	}
#endif
}
